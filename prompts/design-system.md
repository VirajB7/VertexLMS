...
- `package.json` — Next 16.3.2, React 19.2.8, Tailwind v4, TypeScript, ESLint flat config. No UI deps.
...
1. **Tokens live in `@theme` in `app/globals.css`** (Tailwind v4 CSS-first config). No `tailwind.config.ts`. Every value from the sheet becomes a token, so components use `bg-primary-500`, `text-neutral-500`, `shadow-md`, `rounded-md` and never a raw hex.
...
5. **Spacing** — the sheet's base unit is 4px, which is already Tailwind's default `--spacing`. No override; the scale (4/8/12/16/24/32/40/48/64) is documented in the showcase.
6. **Icons: `lucide-react`.** It is a 24×24 grid, 2px stroke, rounded caps set — exactly the icon spec in section 06 — and covers every glyph shown (bell, search, play, file, bookmark, bar-chart, clock, user, chevrons, check-circle, lock, external-link, folder, target, eye, grid, accessibility). Filled style uses the same components with `fill="currentColor"`. This adds one dependency; the alternative is hand-rolling ~18 SVGs and maintaining them.
7. **`cn()` helper** using `clsx` + `tailwind-merge` in `lib/utils.ts`, the standard way to let callers override component classes. Two small dependencies. Variants are plain lookup maps (`Record<Variant, string>`), no `cva` — it is not worth a third dependency at this size.
8. **Components are server components by default.** Only what needs state is `"use client"`: `SearchInput` (controlled value) and `Select` are built uncontrolled/native, so nothing in this task ships client JS except where a consumer needs it. Native `<select>` styled with `appearance-none` plus a chevron — no JS dropdown.
9. **The Vertex logo** is hand-authored SVG (downward orange triangle with the notched inner "V") as `components/brand/logo.tsx`, sized by prop. No image asset.
...
12. **`app/page.tsx` is left as-is** apart from inheriting the new base styles. Replacing the home page is a separate feature and out of scope here.
...
Type scale — Display 1 Playfair 48/56 bold, Display 2 Playfair 36/44 bold, Heading 1 Inter 28/36 semibold, Heading 2 Inter 22/30 semibold, Heading 3 Inter 18/26 medium, Body Large Inter 16/24 regular, Body Inter 14/20 regular, Small Inter 12/16 regular.
...
2. Compare against `design/design/vertex-designsystem.png` top to bottom; each numbered section matches.
